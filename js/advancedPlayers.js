...
function coderLotteryadvancedPlayerse() {
    for (let i = 0; i < 4; i++) {
    ...
    temp.length = 0;
    // for (let j = 0; j < 4; j++) {   //אם ירצו לעקוב בזמן ההגשה   
    //     console.log(temp[j]);
    // }
}


coderLotteryadvancedPlayerse();
...
window.onload = function () {
    ...
    const interval = setInterval(function () {
        const minutesValue = Math.floor(seconds2 / 60);
        const secondsValue = seconds2 % 60;

        timerElement.innerHTML = `${minutesValue}:${secondsValue < 10 ? '0' : ''}${secondsValue}`;
